const data = () => {
  return new Promise((resolve, reject) => {
    fetch('https://1p6ywnb6-6co8s3c1-f37wxsrodk8o.vcd1.mcprev.cn/')
    .then(res => {
...
const init = async () => {
  // let dataTxt = await data()
  // console.log(dataTxt, 'dataTxt')
  let text = document.getElementById('text')
  console.log(text, 'ttt')
  text.innerHTML = `Parcel 是基于资源的，资源可以代表任意文件，并且 Parcel 对 JavaScript，CSS，HTML 文件有更多的支持。 Parcel 会自动地分析这些文件和包中引用的依赖。相同类型的资源会被组合到同一捆绑包中。如果导入其他类型的资源（例如：你在 JS 文件中导入 CSS 文件），Parcel会启动子捆绑包，并在父捆绑包中保留对它的引用。这一点将在以下部分进行说明。
...
如果你想通过内联文件到 JavaScript 包取代 URL 引入文件的方式，你可以使用 Node.js 的 fs.readFileSync API。URL 必须是静态可分析的，意味着它不能有任何变量（除了__dirname 和 __filename）。
...
prev.addEventListener('click', function(){
  if(index === 0) return
  index--;
  article.style.transform = `translateX(${-(width - 16) * index}px)`
})
